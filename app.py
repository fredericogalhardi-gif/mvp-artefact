...
# Se a pessoa não estiver logada na sessão, mas tiver o Cookie no navegador, loga ela direto!
...
def render_login_screen():
    ...
    if st.button("Entrar", type="primary", use_container_width=True):
        ...
        if senha_digitada == senha_correta:
            ...
            # SALVA O COOKIE PARA DURAR 30 DIAS NO NAVEGADOR
            cookie_manager.set("artefact_user", usuario_selecionado, expires_at=datetime.now() + timedelta(days=30))
            time.sleep(0.5) # Tempo rápido pro navegador registrar o cookie
            st.rerun()
        else:
